...
youtube_trailers_link = "http://files.grouplens.org/datasets/movielens/ml-20m-youtube.zip"
...
def download_data(download_path):
    ...
    trailers_path = os.path.join(download_path, 'ml-20m-youtube.zip')
    response = requests.get(youtube_trailers_link, stream=True)
    with open(trailers_path, 'wb') as handle:
        for data in tqdm(response.iter_content()):
            handle.write(data)
    ...
    print("Extracting youtube trailers")
    zip_ref = zipfile.ZipFile(trailers_path, 'r')
...
def load_dataset(input_dataset_path):
    if input_dataset_path is not None:
        ...
        links = pd.merge(imdb_links, youtube_links, on='movieId', how='left')[
...
def fill_database(db_connection, movie_to_movie_similarity, dataset_with_tags, uncomparable_movies, links):
    ...
    links.rename(columns={'movieId': 'movie_id', 'imdbId': 'imdb_id', 'youtubeId': 'youtube_id', 'tmdbId': 'tmdb_id'},
                 inplace=True)
